...
Перед текстом у канал відправляється **квадратне зображення** (кремовий фон, клевер, підпис **завжди українською**: «Гороскоп на 8 квітня»). Дата на картинці та в промпті до Gemini — **календарний день за `Europe/Kyiv`** (не «сьогодні» за UTC на сервері GitHub), щоб число збігалося з Україною. Інший пояс: `HOROSCOPE_TZ`. Російський варіант на зображенні не використовується. Вимкнути картинку: `HOROSCOPE_SKIP_COVER=1`.
