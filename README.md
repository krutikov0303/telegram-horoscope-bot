...
Перед текстом у канал відправляється **квадратне зображення** (кремовий фон, клевер, підпис на кшталт «Гороскоп на 8 квітня» / «Гороскоп на 8 апреля» з **поточною датою** за календарем сервера GitHub — зазвичай збігається з днем публікації). Щоб вимкнути картинку: змінна `HOROSCOPE_SKIP_COVER=1`.
